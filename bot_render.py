...
def restore(target, device_id="*"):
    """Mở khóa trạng thái locked, nhưng KHÔNG gỡ revoked."""
    target = normalize(target)
    device_id = normalize(device_id, 128) or "*"
    with DB_LOCK, db_conn() as db:
        if device_id == "*":
            cur = db.execute(
                "UPDATE clients SET status='active', note='restored', updated_at=? WHERE app=? AND user_id=? AND status='locked'",
                (now_iso(), APP_NAME, target),
            )
        else:
            cur = db.execute(
                "UPDATE clients SET status='active', note='restored', updated_at=? WHERE app=? AND user_id=? AND device_id=? AND status='locked'",
...
def unrevoke(target, device_id="*"):
    """Chỉ admin mới gọi được: gỡ thu hồi khóa cứng."""
    target = normalize(target)
    device_id = normalize(device_id, 128) or "*"
    with DB_LOCK, db_conn() as db:
        if device_id == "*":
            cur = db.execute(
                "UPDATE clients SET status='active', note='UNREVOKED', updated_at=? WHERE app=? AND user_id=? AND status='revoked'",
                (now_iso(), APP_NAME, target),
            )
        else:
            cur = db.execute(
                "UPDATE clients SET status='active', note='UNREVOKED', updated_at=? WHERE app=? AND user_id=? AND device_id=? AND status='revoked'",
                (now_iso(), APP_NAME, target, device_id),
            )
        db.commit()
    return cur.rowcount
...
def command_help():
    return (
        ...
        "/revoke USER_ID - THU HỒI + KHÓA CỨNG\n"
        "/restore USER_ID - mở khóa thường (không gỡ thu hồi)\n"
        "/unrevoke USER_ID - GỠ THU HỒI KHÓA CỨNG\n"
        "/lockdev USER_ID DEVICE_ID - khóa 1 thiết bị\n"
        "/revokedev USER_ID DEVICE_ID - THU HỒI + KHÓA CỨNG thiết bị\n"
        "/restoredev USER_ID DEVICE_ID - mở khóa thường\n"
        "/unrevokedev USER_ID DEVICE_ID - gỡ thu hồi khóa cứng thiết bị\n"
...
def handle_text(chat_id, from_id, text):
    ...
    if cmd in ("/lock", "/unlock", "/revoke", "/restore", "/unrevoke"):
        if len(parts) < 2:
            tg_send(chat_id, f"Cú pháp: {cmd} USER_ID")
            return
        uid = parts[1]
        mapping = {"/lock": "locked", "/unlock": "active", "/revoke": "revoked", "/restore": "active", "/unrevoke": "active"}
        if cmd == "/restore":
            n = restore(uid)
            tg_send(chat_id, f"✅ Đã mở khóa thường cho {uid}. Bản ghi cập nhật: {n}. Thu hồi cứng vẫn giữ nguyên.")
            return
        if cmd == "/unrevoke":
            n = unrevoke(uid)
            tg_send(chat_id, f"✅ Đã GỠ THU HỒI khóa cứng cho {uid}. Bản ghi cập nhật: {n}")
            return
        ok, msg = set_status(uid, mapping[cmd], note=f"Telegram {cmd}")
        prefix = "🔒 " if cmd == "/revoke" else ("✅ " if ok else "❌ ")
        extra = " — KHÓA CỨNG, không thể mở bằng /restore" if cmd == "/revoke" and ok else ""
        tg_send(chat_id, prefix + msg + extra)
        return

    if cmd in ("/lockdev", "/revokedev", "/restoredev", "/unrevokedev"):
        if len(parts) < 3:
        ...
        if cmd == "/restoredev":
            ...
            tg_send(chat_id, f"✅ Mở khóa thường device {device}: {n} bản ghi. Thu hồi cứng vẫn giữ nguyên.")
            return
        if cmd == "/unrevokedev":
            n = unrevoke(uid, device)
            tg_send(chat_id, f"✅ Gỡ thu hồi khóa cứng device {device}: {n} bản ghi")
            return
        status = "locked" if cmd == "/lockdev" else "revoked"
        ok, msg = set_status(uid, status, device_id=device, note=f"Telegram {cmd}")
        extra = " — KHÓA CỨNG, không thể mở bằng /restoredev" if cmd == "/revokedev" and ok else ""
        tg_send(chat_id, ("🔒 " if cmd == "/revokedev" else ("✅ " if ok else "❌ ")) + msg + extra)
        return
