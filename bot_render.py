...
ID_ADMIN = "8801844480"
BOT = "8862931913:AAFLGMCWYYeSE1hbiUCWZoNAs0Ul7oEA8rc"
BOT_TOKEN = BOT
...
def is_admin(user_id: int) -> bool:
    """Chỉ ID_ADMIN được phép quản trị bot."""
    return str(int(user_id)) == ID_ADMIN
...
if bot:
    ...
    def cmd_checkadmin(message):
        ...
        role = "CÓ" if is_admin(message.from_user.id) else "KHÔNG"
        bot.reply_to(message, f"🆔 ID: <code>{html.escape(uid)}</code>\n👑 ADMIN: <b>{role}</b>\n🔐 ID_ADMIN: <code>{html.escape(ID_ADMIN)}</code>")
