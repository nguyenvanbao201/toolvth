...
DB_PATH=os.getenv("DB_PATH","/opt/render/project/src/data/toolxw_files.db")
...
if bot:
    ...
    def _next_file_action_prompt(m, action):
        msg = {
            "lock": "🔴 <b>THU HỒI / KHÓA FILE</b>\n\nGửi <b>FILE_ID</b> cần khóa.\nVí dụ: <code>XWFILE-20260820-A7F3D9K2</code>",
            "unlock": "🟢 <b>MỞ KHÓA FILE</b>\n\nGửi <b>FILE_ID</b> cần mở khóa.\nVí dụ: <code>XWFILE-20260820-A7F3D9K2</code>",
            "info": "🔎 <b>KIỂM TRA FILE</b>\n\nGửi <b>FILE_ID</b> cần kiểm tra.\nVí dụ: <code>XWFILE-20260820-A7F3D9K2</code>",
        }[action]
        sent = bot.send_message(m.chat.id, msg)
        bot.register_next_step_handler(sent, lambda nxt: _handle_next_file_action(nxt, action))

    def _handle_next_file_action(m, action):
        if not admin(m.from_user.id):
            return bot.reply_to(m, "❌ Bạn không có quyền.")
        x = fid((m.text or "").strip())
        if not x or x.startswith('/'):
            return bot.reply_to(m, "❌ FILE_ID không hợp lệ.")
        if action == "lock":
            ok = lock_file(x)
            if ok:
                return bot.reply_to(m, f"✅ <b>ĐÃ THU HỒI / KHÓA FILE</b>\n• FILE ID: <code>{html.escape(x)}</code>\n• Trạng thái: <b>LOCKED</b>")
            return bot.reply_to(m, "❌ FILE_ID chưa tồn tại. Hãy chạy file Toolxw ít nhất 1 lần để server ghi nhận FILE này.")
        if action == "unlock":
            ok = unlock_file(x)
            if ok:
                return bot.reply_to(m, f"✅ <b>ĐÃ MỞ KHÓA FILE</b>\n• FILE ID: <code>{html.escape(x)}</code>\n• Trạng thái: <b>ACTIVE</b>")
            return bot.reply_to(m, "❌ File không ở trạng thái LOCKED hoặc chưa tồn tại.")
        r = get_file(x)
        return bot.reply_to(m, file_text(r))
    ...
    def cb(c):
        ...
        if c.data in {"lock","unlock","info"}:
            _next_file_action_prompt(c.message, c.data)
        else:
            with LOCK,conn() as db:
                rows=db.execute("SELECT file_id,status,device_id,last_seen_at FROM files ORDER BY created_at DESC").fetchall()
            if not rows:
                bot.send_message(c.message.chat.id,"📋 Chưa có file nào kết nối.")
            else:
                text="📋 <b>DANH SÁCH FILE TOOLXW</b>\n"+"\n".join(f"\n{i}. <code>{html.escape(r['file_id'])}</code> — <b>{html.escape(r['status'])}</b>\nDevice: <code>{html.escape(r['device_id'] or '-')}</code>" for i,r in enumerate(rows,1))
    ...
    def revoke_alias(m):
        if not admin(m.from_user.id):return bot.reply_to(m,"❌ Bạn không có quyền.")
        p=m.text.split(maxsplit=1)
        if len(p)<2:return bot.reply_to(m,"❌ Cú pháp: <code>/revoke FILE_ID</code>")
        x=fid(p[1])
        if lock_file(x):
            bot.reply_to(m,f"✅ <b>ĐÃ THU HỒI / KHÓA FILE</b>\n• FILE ID: <code>{html.escape(x)}</code>\n• Trạng thái: <b>LOCKED</b>")
        else:
            bot.reply_to(m,"❌ FILE_ID chưa tồn tại. Hãy chạy file Toolxw ít nhất 1 lần để server ghi nhận FILE này.")

    @bot.message_handler(commands=["restore"])
    def restore_alias(m):
        if not admin(m.from_user.id):return bot.reply_to(m,"❌ Bạn không có quyền.")
        p=m.text.split(maxsplit=1)
        if len(p)<2:return bot.reply_to(m,"❌ Cú pháp: <code>/restore FILE_ID</code>")
        x=fid(p[1])
        if unlock_file(x):
            bot.reply_to(m,f"✅ <b>ĐÃ MỞ KHÓA FILE</b>\n• FILE ID: <code>{html.escape(x)}</code>\n• Trạng thái: <b>ACTIVE</b>")
        else:
            bot.reply_to(m,"❌ File không ở trạng thái LOCKED hoặc chưa tồn tại.")
    def bot_loop():
        while True:
